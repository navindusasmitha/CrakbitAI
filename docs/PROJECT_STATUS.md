...
**Early development / Security MVP alpha + Crakbit Chain v0.36 controlled Proof-of-Work public-testnet integration alpha**
...
2. the primary v0.31+ native Proof-of-Work UTXO path, now at v0.36 controlled public-testnet campaign/integration tooling.
...
| Crakbit Chain package | **v0.36.0a1** | Controlled PoW integration / campaign alpha |
...
## v0.36 Completed Code Work

- package/CLI advanced to `0.36.0a1`,
- signed append-only campaign log for repeated node observations,
- campaign-log signature/hash-chain verification,
- campaign summaries for 24h/72h/7d targets,
- minimum node-count, sample-success-ratio and height-lag gates,
- real RPC probing without synthetic elapsed-time claims,
- incremental UTXO-undo rehearsal on disposable chain copies,
- rollback verification using expected historical state fingerprints,
- peer-book seed selection integrated into the v0.36 node launcher,
- coarse endpoint-bucket diversity limit during seed selection,
- signed algorithm activation-proposal format,
- activation proposal binding to human algorithm decision, source commit, chain ID, genesis and notice window,
- optional consensus-vector and RandomX-library hash binding,
- explicit `consensus_activated=false` / production-boundary semantics,
- regression coverage for campaign logs, undo rehearsal, peer selection and activation proposals,
- successful CI for the v0.36 regression-code commit.

## Important Boundary

v0.36 can collect and verify real observation evidence, but it cannot create independent infrastructure or make time pass. A 24h/72h/7d campaign is only satisfied after the real nodes have actually operated for that duration and the signed observations support it.
...
## Immediate Priorities — v0.37 target
...
8. Harden pool payout operations with hot/cold separation, payout caps/holds and reconciliation evidence.
9. Expand explorer/wallet confirmation and reorg UX.
10. Build an independent-review handoff bundle from actual public-testnet evidence.
...
See `blockchain/V0.36.md`, `blockchain/README.md`, `ROADMAP.md` and GitHub Issue #1.
