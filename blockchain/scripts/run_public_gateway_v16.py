...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Run the hardened Crakbit v0.16 public wallet/explorer gateway"
    )
    ...
    parser.add_argument("--rate-limit-db", default="runtime/gateway-rate-limits.sqlite3")
    parser.add_argument("--write-rpm", type=int, default=60)
    ...
    os.environ["CRAKBIT_GATEWAY_EXPLORER_URL"] = args.explorer_url
    os.environ["CRAKBIT_GATEWAY_FAUCET_URL"] = args.faucet_url
    os.environ["CRAKBIT_GATEWAY_MINING_URL"] = args.mining_url
    os.environ["CRAKBIT_GATEWAY_ALLOWED_ORIGINS"] = ",".join(args.allowed_origin)
    os.environ["CRAKBIT_GATEWAY_RATE_LIMIT_DB"] = args.rate_limit_db
    os.environ["CRAKBIT_GATEWAY_WRITE_RPM"] = str(args.write_rpm)

    from crakbit_chain.public_gateway_v16 import create_app
